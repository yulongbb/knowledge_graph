...
export class PdfComponent extends PageBase {
  ...
  constructor(
    ...
    this.activatedRoute.paramMap.subscribe((x: ParamMap) => {
      this.data$ = this.service
        .getList(this.index, this.size, { collection: 'entity', type: 'PDF', keyword: `%${this.keyword}%` })
        .pipe(
          tap((x: any) => console.log(x)),
          map((x: any) => x)
        );
      this.data = (index: number, size: number, query: Query) => this.service
        .getList(index, this.size, { collection: 'entity', type: 'PDF', keyword: `%${this.keyword}%` })
        .pipe(
          tap((x: any) => console.log(x)),
          map((x: any) => x)
        );
  ...
  search(keyword: any) {
    this.data$ = this.service
      .getList(this.index, this.size, { collection: 'PDF_entity', type: 'PDF', keyword: `%${this.keyword}%` })
      .pipe(
        tap((x: any) => console.log(x)),
        map((x: any) => x)
      );
    this.data = (index: number, size: number, query: Query) =>
      this.service.getList(index, this.size, { collection: 'PDF_entity', type: 'PDF', keyword: `%${keyword}%` }).pipe(
        tap((x: any) => console.log(x)),
        map((x: any) => x)
      );
  }
