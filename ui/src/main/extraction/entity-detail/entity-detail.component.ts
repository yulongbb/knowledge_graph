  ...
  ngOnInit(): void {
    ...
    this.esService.searchEntity(1,10, {bool:query}).subscribe((data: any) => {
      ...
      if (data.list[0]) {
        ...
        this.ontologyService.get(data.list[0]._source.type).subscribe((type: any) => {
          console.log(type);
          this.form.formGroup.patchValue({ _key: data.list[0]?._id, label: this.item?.labels?.zh?.value, type: { id: type?.id, label: type?.name }, description: this.item.descriptions?.zh?.value });
