...
export class TabsComponent implements OnInit {
  get activatedIndex() {
    return this.indexService.session.tabsPage
      ? this.indexService.session.tabsPage.findIndex(
          (x) => x.router === this.indexService.session.activatedPage
        )
      : -1;
  }
  ...
  get showClose() {
    return this.indexService.session.tabsPage &&
      this.indexService.session.tabsPage.length > 1
      ? true
      : false;
  }
   ...
   */
  tab(index: number) {
    const tab = this.indexService.session.tabsPage?.find(
      (_x, i) => i === index
    );
    if (tab && tab.router) {
      let page = tab.router;
