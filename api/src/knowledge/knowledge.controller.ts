...
export class KnowledgeController {
  ...
  @ApiOperation({
    summary: '添加知识实体',
    description: '创建一个新的知识实体到系统中',
  })
  @ApiBody({
    description: '知识实体数据，包含标签、描述、别名等信息',
    type: Entity,
  })
  ...
  @ApiBody({
    description: '搜索条件，Elasticsearch查询DSL格式',
    type: SearchQuery,
  })
  @ApiResponse({
    status: 200,
    description: '搜索结果，包含实体列表和聚合数据',
    type: SearchResponse,
  })
  async searchEntity(
  ...
  @ApiOperation({
    summary: '获取知识实体详情',
    description: '根据ID获取知识实体的详细信息',
  })
  @ApiParam({ name: 'id', description: '知识实体的唯一标识' })
  @ApiResponse({
    status: 200,
    description: '知识实体详细信息，包含自动提取的实体',
    type: EntityWithExtracted,
  })
  ...
  @ApiBody({
    description: '关系信息，包含源实体、目标实体和关系类型',
    type: Edge,
  })
  ...
  @ApiOperation({
    summary: '获取知识图谱',
    description:
      '获取以指定实体为中心的知识图谱，支持分页，返回适合可视化的数据结构',
  })
  ...
  @ApiBody({
    description: '批处理任务数据，包含类型和实体ID列表',
    type: CreateBatchTaskDto,
  })
  @ApiResponse({
    status: 201,
  ...
  async createBatchTask(
    @Body() createTaskDto: CreateBatchTaskDto,
  ...
  @ApiResponse({
    status: 200,
    description: '批处理任务更新成功',
    type: BatchTaskResponse,
  })
  async updateBatchTask(
    @Body() updateTaskDto: UpdateBatchTaskDto,
  ): Promise<BatchTaskResponse> {
    try {
